...
def SMGWO(Positions,objf, lb, ub, dim, SearchAgents_no, Max_iter):
    #F = 0.6  # 缩放因子
    T = 1
    T0 = 1
    K = 0.9
...
for fun in range(2,len(func_details)):
    ...
    for i in range(tn):
        ...
        x ,vec= SMGWO(positions_1,Fx, lb, ub, dim, SearchAgents_no, Max_iter)#改进灰狼
        end_time = time()
    ...
    for i in range(tn):
        ...
        vm ,vec= vm_GWO(positions_5,Fx, lb, ub, dim, SearchAgents_no, Max_iter)
        end_time = time()
        run_time = end_time - begin_time
    ...
    output_path = outfile
    with open(output_path, 'a', encoding='utf-8') as file1:
        print(f'变权灰狼平均值{np.mean(VM)}标准差{np.std(VM, ddof=1)}',file=file1)
        print(file=file1)
    print()